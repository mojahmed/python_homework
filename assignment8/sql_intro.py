...
try:
    ...
    for m in magazines:
        c.execute("INSERT INTO magazines (name, publisher_id) VALUES ('" + m[0] + "', " + str(m[1]) + ")")

    # Subscribers
    subscribers = [("John", "123 Main St"), ("Ahmed", "45 King Rd"), ("Mary", "78 Elm St")]
    for s in subscribers:
        c.execute("INSERT INTO subscribers (name, address) VALUES ('" + s[0] + "', '" + s[1] + "')")

    # Subscriptions
    subscriptions = [(1, 1, "2025-12-31"), (2, 2, "2025-11-15"), (3, 3, "2025-10-01")]
    for sub in subscriptions:
        c.execute("INSERT INTO subscriptions (subscriber_id, magazine_id, expiration_date) VALUES (" + str(sub[0]) + ", " + str(sub[1]) + ", '" + sub[2] + "')")
    ...
    c.execute("SELECT magazines.id, magazines.name, publishers.name FROM magazines JOIN publishers ON magazines.publisher_id = publishers.id WHERE publishers.name = 'Tech World'")
    for row in c.fetchall():
        print(row)
    print()
